...
<p class="subtitle">[Subtitle description]</p>
    ...
    <!-- ARCHIFY:SOURCE_SLOT_START -->    <!-- ARCHIFY:SOURCE_SLOT_END -->
[Project Name] &bull; [Additional metadata]`;
  const html = applyTemplate(template, {
    ...
    svg: '<svg/>',
    cards: '',
    // Dollar sequences inside the embedded source must survive the function
    // replacer too — `$&` in a label is a classic replacement-pattern trap.
    source: { type: 'workflow', json: { meta: { title: 'Plan $& $$50' } }, version: '0.0.0' },
  });
  assert.match(html, /Plan \$\$50 tier/);
  assert.match(html, /"Plan \$& \$\$50"/);
});
