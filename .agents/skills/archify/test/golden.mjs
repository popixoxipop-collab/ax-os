...
// A development checkout keeps the golden HTML in <repo>/examples one level
// above the skill; a standalone skill install has no parent repo, so fall
// back to the fixtures checked in under test/golden.
const repoExamples = path.join(repoRoot, 'examples');
const goldenDir = fs.existsSync(repoExamples) ? repoExamples : path.join(skillRoot, 'test/golden');
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    const checked = fs.readFileSync(path.join(goldenDir, golden), 'utf8');
    check(`${mode}: ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(checked),
      `fresh render differs from ${path.join(goldenDir, golden)}; if the change is intentional, re-render the examples and commit them`);
  } catch (err) {
...
const webApp = fs.readFileSync(path.join(goldenDir, 'web-app.html'), 'utf8');
...
// The embedded #archify-source block exists only in rendered output (the
// template ships an empty ARCHIFY:SOURCE_SLOT), so drop it before comparing.
const notEmbeddedSource = (b) => !b.includes('id="archify-source"');
for (const tag of ['style', 'script']) {
  const t = blocks(template, tag).filter((b) => !b.includes('[PROJECT NAME]')).filter(notEmbeddedSource);
  const w = blocks(webApp, tag).filter((b) => !b.includes('Sample Web App')).filter(notEmbeddedSource);
