...
test('cli: inspect emits architecture layout json', (t) => {
  // The grid-mode fixture ships with the development repo one level above the
  // skill; a standalone skill install does not carry it.
  const input = path.resolve(skillRoot, '../examples/archify-repo-grid.architecture.json');
  if (!fs.existsSync(input)) return t.skip('development-repo grid example not present in this install');
