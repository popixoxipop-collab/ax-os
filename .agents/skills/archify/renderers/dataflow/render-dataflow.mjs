...
// Stages have no source id (schema: label only) — they are addressed by index.
function renderStage(stage, index) {
  const cx = stageX(index);
  const x = cx - layout.stageW / 2;
  const h = viewBox[1] - layout.stageY - layout.stageBottomPad;
  return archWrap(`stage:${index}`, 'stage', 'body', `        <rect x="${x}" y="${layout.stageY}" width="${layout.stageW}" height="${h}" rx="10" class="c-lane" stroke-width="1"/>
...
// Flow paths draw before nodes and flow labels after them (z-order rule), so
// one flow becomes two clusters sharing a derived id and differing by part.
function renderFlowPath(flow, index) {
  ...
  return archWrap(archEdgeId(flow, index), 'flow', 'path', `        <path d="${routed.d}" class="${cls}"${animateAttr(dataflow.meta, 'edge', index)} stroke-width="${strokeWidth}" marker-end="url(#${marker})"/>`);
}
...
function renderFlowLabel(flow, index) {
  ...
  return archWrap(archEdgeId(flow, index), 'flow', 'label', `        <rect x="${lx - labelW / 2}" y="${ly - 11}" width="${labelW}" height="${labelH}" rx="4" class="c-mask"/>
