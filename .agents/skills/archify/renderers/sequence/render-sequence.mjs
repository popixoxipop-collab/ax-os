...
import { esc, renderDefinitions, textUnits, archWrap, archMessageId } from '../shared/utils.mjs';
...
function renderParticipant(participant) {
  ...
  return archWrap(participant.id, 'participant', 'body', `        <rect x="${participant.x}" y="${layout.topY}" width="${layout.participantW}" height="${layout.participantH}" rx="6" class="c-mask"/>
...
// A lifeline is the 1:1 timeline artifact of its participant, so it reuses the
// participant's source id under its own kind.
function renderLifeline(participant) {
  return archWrap(participant.id, 'lifeline', 'body', `        <path d="M ${participant.cx} ${layout.lifelineTop} L ${participant.cx} ${layout.lifelineBottom}" class="a-default" stroke-width="0.8" stroke-dasharray="3,7"/>`);
}

function renderSegment(segment, index) {
  return archWrap(`seg:${index}`, 'segment', 'body', `        <rect x="48" y="${segment.from}" width="${viewBox[0] - 96}" height="${segment.to - segment.from}" rx="10" class="c-lane" stroke-width="1"/>
        <text x="62" y="${segment.from + 18}" class="t-dim" font-size="9" font-weight="600">${esc(segment.label)}</text>`);
}

function renderActivation(activation, index) {
  ...
  return archWrap(`act:${index}`, 'activation', 'body', `        <rect x="${x}" y="${activation.from}" width="10" height="${height}" rx="3" class="c-mask"/>
...
// A message's arrow, label, and note are emitted as one contiguous cluster,
// so a single wrapper (part "body") covers the whole logical element.
function renderMessage(message, index) {
  ...
  const note = message.note
    ...
    : '';
  return archWrap(archMessageId(message, index), 'message', 'body', `        <path d="M ${start} ${message.y} L ${end} ${message.y}" class="${cls}"${animateAttr(sequence.meta, 'edge', index)} stroke-width="${strokeWidth}"${dash} marker-end="url(#${marker})"/>
