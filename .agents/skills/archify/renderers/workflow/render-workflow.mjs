...
function renderLane(lane, index) {
  ...
  return archWrap(lane.id, 'lane', 'body', `        <rect x="${layout.laneX}" y="${y}" width="${layout.laneW}" height="${layout.laneH}" rx="10" class="c-lane" stroke-width="1"/>${exception}
...
function renderPhase(phase) {
  ...
  return archWrap(phase.id, 'phase', 'body', `        <line x1="${span.x}" y1="35" x2="${span.x + span.width}" y2="35" class="${lineClass}" stroke-width="1.1"/>
...
function renderGroup(group) {
  ...
  return archWrap(group.id, 'group', 'body', `        <rect x="${span.x}" y="${y}" width="${span.width}" height="${height}" rx="9" class="${cls}" stroke-width="1"/>
...
function renderNode(node) {
  ...
  const tag = node.tag
    ...
    : '';
  return archWrap(node.id, 'node', 'body', `        <rect x="${node.x}" y="${node.y}" width="${node.width}" height="${node.height}" rx="6" class="c-mask"/>
...
// Edge paths draw before nodes and edge labels after them (z-order rule), so
// one edge becomes two clusters sharing a derived id and differing by part.
function renderEdgePath(edge, index) {
  ...
  return archWrap(archEdgeId(edge, index), 'edge', 'path', `        <path d="${routed.d}" class="${cls}"${animateAttr(workflow.meta, 'edge', edgeSteps.get(edge))} stroke-width="${strokeWidth}" marker-end="url(#${marker})"/>`);
}
...
function renderEdgeLabel(edge, index) {
  ...
  return archWrap(archEdgeId(edge, index), 'edge', 'label', `        <rect x="${lx - labelW / 2}" y="${ly - 10}" width="${labelW}" height="14" rx="3" class="c-mask"/>
