...
// ---- Rendering ---------------------------------------------------------------
// Boundaries have no source id (schema: kind/label/wraps/pad), so they get a
// derived index id following the same convention as the other id-less arrays.
function renderBoundary(b, index) {
  ...
  return archWrap(`boundary:${index}`, 'boundary', 'body', `        <rect x="${b.x}" y="${b.y}" width="${b.width}" height="${b.height}" rx="${rx}" class="${cls}" stroke-width="1"/>
        <text x="${b.x + 8}" y="${b.y + 18}" class="${labelCls}" font-size="9" font-weight="600">${esc(b.label)}</text>`);
}

// Connection paths draw before components and labels after them (z-order
// rule), so one connection is two clusters: same derived id, parts path/label.
function renderConnectionPath(conn, index) {
  ...
  return archWrap(archEdgeId(conn, index), 'connection', 'path', `        <path d="${routed.d}" class="${cls}"${animateAttr(arch.meta, 'edge', index)} stroke-width="${strokeWidth}" marker-end="url(#${marker})"/>`);
}
...
function renderConnectionLabel(conn, index) {
  ...
  const [lx, ly] = labelPoint(conn, pathFor(conn).points);
  const w = Math.max(30, textUnits(conn.label) * 4.8 + 10);
  return archWrap(archEdgeId(conn, index), 'connection', 'label', `        <rect x="${lx - w / 2}" y="${ly - 10}" width="${w}" height="14" rx="3" class="c-mask"/>
...
function renderComponent(c) {
  ...
  const tag = c.tag
    ...
    : '';
  return archWrap(c.id, 'component', 'body', `        <rect x="${c.x}" y="${c.y}" width="${c.width}" height="${c.height}" rx="6" class="c-mask"/>
