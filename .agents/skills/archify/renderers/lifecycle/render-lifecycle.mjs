...
import { esc, renderDefinitions, textUnits, archWrap, archEdgeId } from '../shared/utils.mjs';
...
// The three bands are fixed renderer structure (phase/event/outcome), not a
// source array, so each gets a stable semantic id instead of an index.
function renderBands() {
  const right = viewBox[0] - 72;
  const titles = bandTitles();
  const bands = [
    ['band:phase', 112, 100, '01', titles[0]],
    ['band:event', 264, 252, '02', titles[1]],
    ['band:outcome', 436, 424, '03', titles[2]],
  ];
  return bands.map(([id, lineY, textY, prefix, title]) => archWrap(id, 'band', 'body',
    `        <path d="M 72 ${lineY} L ${right} ${lineY}" class="a-default" stroke-width="0.8" stroke-dasharray="3,8"/>
        <text x="72" y="${textY}" class="t-dim" font-size="10" font-weight="600">${prefix} / ${esc(title)}</text>`)).join('\n');
}
...
function renderState(state) {
  ...
  const step = state.step
    ...
    : '';
  return archWrap(state.id, 'state', 'body', `        <rect x="${state.x}" y="${state.y}" width="${state.width}" height="${state.height}" rx="7" class="c-mask"/>
...
// Transition paths draw before states and labels after them (z-order rule),
// so one transition is two clusters: same derived id, parts path/label.
function renderTransitionPath(transition, index) {
  ...
  return archWrap(archEdgeId(transition, index), 'transition', 'path', `        <path d="${routed.d}" class="${cls}"${animateAttr(lifecycle.meta, 'edge', index)} stroke-width="${strokeWidth}" marker-end="url(#${marker})"/>`);
}
...
function renderTransitionLabel(transition, index) {
  ...
  const note = transition.note
    ...
    : '';
  return archWrap(archEdgeId(transition, index), 'transition', 'label', `        <rect x="${lx - labelW / 2}" y="${ly - 11}" width="${labelW}" height="${labelH}" rx="4" class="c-mask"/>
...
function renderLifecycleRail() {
  ...
  return archWrap('rail', 'rail', 'body', `        <path d="M 154 ${layout.phaseY + 31} L ${railEnd} ${layout.phaseY + 31}" class="a-emphasis" stroke-width="2.2" marker-end="url(#arrowhead-emphasis)"/>`);
}
...
${asArray(lifecycle.transitions).map(renderTransitionLabel).join('\n')}
        ...
        <!-- Legend -->
${archWrap('legend', 'legend', 'body', renderLegend())}
      </svg>`;
}
