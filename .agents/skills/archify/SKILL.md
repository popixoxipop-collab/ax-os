...
### Hybrid custom-shape injection (renderer layout + hand-placed geometry)

D1: the typed renderers' `componentType`/`node.type` enum (`frontend`/`backend`/`database`/`cloud`/`security`/`messagebus`/`external`) is a fixed set of rectangles — there is no schema field for a non-rectangular shape (a logic gate, a non-standard glyph). Two ways to get one into a diagram: hand-place the whole SVG (the fallback above, forfeiting lanes/legend/theme-toggle machinery), or **inject the custom shape into an otherwise-normal renderer output**. The second is usually better when only a handful of nodes need the unusual shape and everything else — lanes, phase headers, legend, dark/light theme — should keep working exactly as the renderer already provides it.
  - WHY: rebuilding lane layout, legend generation, and the two-rect mask/theme-var system by hand (full hand-placed fallback) is a lot of surface to get right for what's often 2-10 outlier nodes in an otherwise standard diagram.
  - COST: injected shapes bypass the renderer's own layout/overlap guards entirely (node/label collision checks, off-canvas checks) — the two checkers above become mandatory, not optional, for any diagram that uses this technique.
  - EXIT: if a renderer mode ever grows a real custom-path component type, prefer that over injection for new diagrams; this technique stays relevant for one-off shapes not worth a schema addition.

**Technique**:
1. Render normally, using an existing component/node type as a placeholder for each node that actually needs the custom shape (pick whichever type gives the closest box size/position).
2. In the rendered HTML, find the placeholder's two-rect pattern — `<rect class="c-mask" x=".." y=".." width=".." height="..">` immediately followed by `<rect class="c-<type>" ...>` with **identical geometry** (this is the Cardinal Rule pattern every component emits; see Hard layout rules above).
3. Replace both rects with a custom path wrapped in `<g transform="translate(X,Y) scale(w/baseW,h/baseH)">`, where `X,Y` are the placeholder rect's own `x,y` and `baseW,baseH` are the dimensions your path was authored at (so it scales into whatever box the renderer laid out, instead of hand-computing coordinates per instance). **Keep the original `c-mask`/`c-<type>` classes on the replacement shapes** — this is what keeps the theme toggle working (Cardinal Rule: no hardcoded colors); only the geometry changes, never the class.
4. Re-run `scripts/check-line-overlaps.mjs` and `scripts/check-text-overlaps.mjs` (per COST above, the renderer's own guards never saw this geometry).
5. Export: Playwright, `deviceScaleFactor: 2`, screenshot only the diagram's own container element (not the full page/wrapper) for a clean, tight-cropped raster.

**Case study**: a 30+ node, 3-lane service-flow diagram needed 10 AND/OR logic-gate nodes the `workflow` renderer's node types don't cover. Rendered the lane/phase/legend skeleton normally with placeholder rect types, then swapped 10 of them for gate paths via the technique above, keeping their original component classes so both themes still render correctly. Both overlap checkers went into the loop specifically because this diagram's first pass had real hand-injection defects the renderer never had a chance to catch.

**Readability lesson (not caught by either checker)**: zero findings from both overlap checkers means no *geometric* overlap — it does not mean every label's association with its shape is unambiguous at a glance. In one dense cluster, a gate's caption sat close enough to an *unrelated* downstream gate that a quick look could misattribute it, even though tracing the actual connector line showed it was wired correctly and no checker flagged anything (nothing overlapped — the two shapes were just close together). Geometric-overlap checks and human-readable disambiguation are different properties; in a dense cluster, zoom in and trace the actual connector lines by eye after the checkers pass clean, don't stop at "0 findings."
