...
function usage() {
  ...
  archify serve [--port N] [--dir editor]
  archify examples
  archify doctor
...
// `archify serve` — local render service for the element editor (plan §6 S4).
// Zero-dependency node:http: statically serves editor/ and exposes POST
// /render|/validate|/check that re-run the same renderer/check code path as the
// CLI. Additive: touches none of the existing subcommands. Class (a) editing
// needs this local Node path because renderers are spawn-per-run scripts (G4).
async function commandServe(args) {
  let port = 4517;
  let dir = path.join(skillRoot, 'editor');
  for (let i = 0; i < args.length; i += 1) {
    if (args[i] === '--port') {
      const value = Number.parseInt(args[i + 1], 10);
      if (!Number.isInteger(value) || value < 0 || value > 65535) fail(`Invalid --port "${args[i + 1]}".`);
      port = value;
      i += 1;
    } else if (args[i] === '--dir') {
      if (!args[i + 1]) fail('--dir requires a directory path.');
      dir = path.resolve(args[i + 1]);
      i += 1;
    } else {
      fail(`Unknown serve option "${args[i]}".\n\n${usage()}`);
    }
  }

  const serverModule = path.join(skillRoot, 'editor/server.mjs');
  if (!fs.existsSync(serverModule)) fail(`Missing editor server module: ${serverModule}`, 1);
  const { startServer } = await import(pathToFileURL(serverModule).href);
  try {
    const { url } = await startServer({ port, dir });
    console.log(`archify serve → ${url}`);
    console.log(`  editor dir: ${dir}`);
    console.log('  POST /render {type,source} · POST /validate {type,source} · POST /check {html}');
    console.log('  Ctrl+C to stop.');
  } catch (error) {
    fail(`Could not start server on port ${port}: ${error.message}`, 1);
  }
}
