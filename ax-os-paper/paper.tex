...
$+16.2\%$, $+14.4\%$, $+14.3\%$---scale alone does not drive quantization
robustness.  Cross-architecture variation dominates: Mistral-7B degrades only
$+3.9\%$ versus Qwen2.5-7B's $+14.3\%$ at the same $3.5\times$ compression
ratio, a $3.7\times$ gap that motivates architecture-stratified rather than
...
\begin{enumerate}
  \item \textbf{Scale-stratified 4-bit quantization measurements (primary).}
    ...
    ($+16.2\%$ at 1.5B to $+14.3\%$ at 7B), while Mistral-7B degrades only
    $+3.9\%$---architecture dominates scale as a predictor of quantization
    robustness at constant $3.5\times$ compression.
  ...
  \item \textbf{Inline BRAIN\_REAL oracle switching (enabling).}
    An environment-variable gate activates a Python subprocess oracle without
    a compiled \texttt{dist/} artifact, preserving demo-script portability on
...
\textbf{Quantization implementation differences (controlled).}  The Qwen q4
models in \cref{tab:scale} were obtained from mlx-community (group size
unspecified) while the Mistral q4 artifact was produced locally with
\texttt{mlx\_lm.convert} (group size 64).  To rule out a pipeline confound, we
re-quantized Qwen2.5-7B with the \emph{identical} local pipeline (group size 64)
and re-measured WikiText-2 perplexity under the same protocol: it degraded
$+16.3\%$ (ppl $9.22\!\rightarrow\!10.72$), marginally \emph{more} than the
mlx-community artifact ($+14.3\%$) and far more than Mistral-7B ($+3.9\%$) at
identical compression.  Implementation choice therefore moves $\Delta$PPL by
$\sim$2 points---an order of magnitude smaller than the $\sim$12-point
cross-architecture gap, which survives identical pipelines.  A residual
$\sim$2-point sensitivity to group size and conversion tooling is the one
implementation caveat that remains.
...
family is nearly flat ($+16.2\%$ at 1.5B, $+14.4\%$ at 3B, $+14.3\%$ at 7B)
at $3.5\times$ compression---scale alone is a weak predictor of quantization
robustness in the 1.5B--7B range.  Cross-architecture variation is far larger:
Mistral-7B degrades only $+3.9\%$ at identical scale and compression, a
$3.7\times$ gap that motivates architecture-stratified rather than scale-only
