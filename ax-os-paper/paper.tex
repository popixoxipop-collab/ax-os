...
to sub-3B models: standard WikiText-2 evaluation shows Qwen2.5 ΔPPL is
...
\begin{table}[t]
  ...
  \caption{Scale-stratified uniform q4 compression on WikiText-2 (Apple M1 Max, MLX).
    Within the Qwen family, ΔPPL is nearly flat across 1.5B–7B.
...
Within the Qwen family, ΔPPL decreases only slightly across three scale points
...
\begin{figure}[t]
  ...
  \caption{Perplexity increase (\%) under uniform q4 quantization versus
    ...
    ΔPPL is nearly flat: $+16.2\%$ (1.5B), $+14.4\%$ (3B), $+14.3\%$ (7B).
...
family, uniform q4 ΔPPL is nearly scale-invariant: $+16.2\%$, $+14.4\%$,
...
intra-family finding (flat ΔPPL across 1.5B–7B Qwen) rests on three
data points; 13B and 34B measurements are needed to determine whether the
plateau continues or ΔPPL eventually declines.  The cross-architecture
...
gap in ΔPPL and should be controlled in future work.
...
and Mistral-7B shows that within the Qwen family, uniform q4 ΔPPL is nearly
...
checkpoints to determine whether the Qwen ΔPPL plateau continues, controlling
