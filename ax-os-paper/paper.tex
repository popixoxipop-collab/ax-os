...
uniform q4 (group size 64) perplexity degradation decreases monotonically with
scale: $+15.0\%$ (1.5B), $+11.7\%$ (3B), $+8.5\%$ (7B), indicating that scale
is a moderate predictor within this family.  Cross-architecture variation at
...
\begin{enumerate}
  \item \textbf{Scale-stratified 4-bit quantization measurements (primary).}
    ...
    shows that within the Qwen family, uniform q4 $\Delta$PPL decreases
    monotonically with scale ($+15.0\%$ at 1.5B, $+11.7\%$ at 3B, $+8.5\%$ at 7B),
    while Mistral-7B degrades only $+4.4\%$ at matched 7B scale---a $1.9\times$
    architecture-driven gap beyond the scale trend, at $3.5\times$ compression, within
...
\begin{table}[t]
  ...
  \caption{Scale-stratified uniform q4 compression on WikiText-2 (Apple M1 Max, MLX).
    Within the Qwen family, $\Delta$PPL decreases monotonically: $+15.0\%$ (1.5B),
    $+11.7\%$ (3B), $+8.5\%$ (7B).  At matched 7B scale, architecture provides
...
Within the Qwen family, $\Delta$PPL decreases monotonically across scale
(1.5B: $+15.0\%$, 3B: $+11.7\%$, 7B: $+8.5\%$)---a $1.8\times$ range over a
$4.7\times$ parameter increase, consistent with larger models developing more
quantization-robust representations.
...
\begin{figure}[t]
  ...
  \caption{Perplexity increase (\%) under uniform q4 quantization versus
    ...
    $+8.5\%$ (7B). Mistral-7B (cross marker) degrades only $+4.4\%$ at
    the same 7B scale, a $1.9\times$ cross-architecture gap.  All models share
...
family, uniform q4 $\Delta$PPL decreases monotonically with scale: $+15.0\%$,
$+11.7\%$, $+8.5\%$ at 1.5B, 3B, and 7B respectively---a $1.8\times$ intra-family
range over a $4.7\times$ parameter increase.  Second, at matched 7B scale,
...
intra-family finding (monotonically decreasing $\Delta$PPL across 1.5B–7B Qwen) rests on three
data points; 13B and 34B measurements are needed to determine whether the
decrease continues or $\Delta$PPL eventually plateaus.  The cross-architecture
...
family decreases monotonically ($+15.0\%$ at 1.5B, $+11.7\%$ at 3B,
$+8.5\%$ at 7B) at $3.5\times$ compression---a $1.8\times$ intra-family range
indicating scale is a moderate predictor.  At matched 7B scale, cross-architecture
...
checkpoints to determine whether the Qwen $\Delta$PPL monotonic decrease continues,
