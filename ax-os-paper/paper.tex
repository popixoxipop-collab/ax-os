...
$+3.9\%$ versus Qwen2.5-7B's $+13.5\%$–$+16.3\%$ at the same $3.5\times$
compression ratio, a $3.5\times$–$4.2\times$ gap that motivates
architecture-stratified rather than scale-only quantization decisions.  Mixed-precision alternatives (3-bit low
...
The cross-architecture conclusion is robust to the short-corpus uncertainty
in the 7B estimate: at $+13.5\%$ (short-corpus) the gap is $3.5\times$;
at $+16.3\%$ (companion full-corpus, \cref{sec:results:limits}) it is
$4.2\times$---both far exceeding the $1.3\times$ intra-family range and
ruling out a scale explanation for the cross-architecture gap.
...
far larger: Mistral-7B degrades by only $+3.9\%$, a $3.5\times$–$4.2\times$ gap versus
Qwen2.5-7B at the same scale and compression ratio (range reflects the
$+13.5\%$ short-corpus and $+16.3\%$ full-corpus estimates for 7B).  These results suggest that,
