#!/usr/bin/env python3
"""
Track G driver: swap RealFreqDomainBottleneck (G1) / ComplexChannelPairBottleneck
(G2) into Qwen2.5-1.5B-Instruct's FFN at L14 (model.model.layers[14].mlp),
measure C4 PPL zero-shot (noop + calibrated init) and after a short fine-tune.
See eval/bottleneck_ablation.py for the module implementations and design
rationale (D5-D8), and ../SPECTRAL_LLM_SURGERY_FINDINGS.md for the Track A-F
context this 2x2 ablation follow-up tests.

Mirrors scripts/run_bottleneck_experiment.py (Track F) exactly in procedure
and hyperparameters (epochs=1, lr=1e-3, Adam, C4 doc slices 200-400 train /
0-200 eval, stride=512). Does not import from or modify run_bottleneck_experiment.py
or bottleneck_filter.py -- new files only, per Track G scope constraints.
"""
import argparse
import json
import os
...
from bottleneck_ablation import (  # noqa: E402
    RealFreqDomainBottleneck, ComplexChannelPairBottleneck, _verify_pack_unpack_roundtrip,
)
from bottleneck_filter import calibrate_init_std_bottleneck  # noqa: E402  (generic helper, reused unmodified)
...
LAYER = 14  # D4 in bottleneck_filter.py (Track F), reused unchanged for Track G
RANK = 8    # D8 in bottleneck_ablation.py: matches F2's 24,576-param budget
TRACK = "A"  # get_target(model, "A", layer) -> (block, "mlp", block.mlp)
ARTIFACTS_DIR = os.path.join(_HERE, "..", "artifacts")
ORIG_FFN_PARAMS = 3 * DIM * 8960  # gate_proj + up_proj + down_proj

ARMS = {
    "G1": RealFreqDomainBottleneck,
    "G2": ComplexChannelPairBottleneck,
}
...
def main():
    ...
    ap.add_argument("--arm", type=str, default="G1", choices=["G1", "G2"])
    args = ap.parse_args()

    os.makedirs(ARTIFACTS_DIR, exist_ok=True)

    print("Verifying pack/unpack roundtrip...", flush=True)
    ...
    if args.phase == "dryrun":
        ...
        results = []
        arms_to_run = [args.arm] if args.arm else ["G1", "G2"]
...
def _save(results, tag, final=False):
    path = os.path.join(ARTIFACTS_DIR, f"bottleneck_trackG_results_{tag}.json")
